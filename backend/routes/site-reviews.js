...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    // Notify admins
    try {
      const typeLabel = { review:'reseña', complaint:'queja', suggestion:'sugerencia' }[type||'review'] || 'mensaje';
      const { sendToAdmins } = require('./push');
      await sendToAdmins({
        title: `💬 Nueva ${typeLabel} del sitio`,
        body: comment?.slice(0,80) || 'Nuevo mensaje recibido',
