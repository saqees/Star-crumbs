...
const ensureColumns = async () => {
  try {
    await pool.query(`
      ...
      ALTER TABLE users ADD COLUMN IF NOT EXISTS location_type VARCHAR(20) DEFAULT 'sena';
      ALTER TABLE users ADD COLUMN IF NOT EXISTS specific_address TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS location_map_url TEXT;
    `);
...
router.put('/me', auth, async (req, res) => {
  const { full_name, phone, ambiente_number, schedule, username, profile_picture, location_type, specific_address, location_map_url } = req.body;
  try {
    const r = await pool.query(
      `UPDATE users SET
        ...
        location_type=COALESCE($8,location_type), specific_address=$9, location_map_url=$10,
        updated_at=NOW()
       WHERE id=$7
       RETURNING id,username,email,full_name,phone,ambiente_number,schedule,profile_picture,role,location_type,specific_address,location_map_url`,
      [full_name, phone, ambiente_number, schedule, username, profile_picture, req.user.id, location_type, specific_address||null, location_map_url||null]
    );
    res.json(r.rows[0]);
