...
const ensureColumns = async () => {
  try {
    await pool.query(`
      ...
      ALTER TABLE orders ADD COLUMN IF NOT EXISTS location_type VARCHAR(20);
      ALTER TABLE orders ADD COLUMN IF NOT EXISTS location_detail TEXT;
    `);
...
router.post('/', auth, async (req, res) => {
  const { payment_method, notes, allergies, receipt_url, location_type, location_detail } = req.body;
  ...
  try {
    ...
    const order = await client.query(
      `INSERT INTO orders (user_id, total, payment_method, notes, allergies, receipt_url, location_type, location_detail) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
      [req.user.id, total, payment_method, notes, allergies||null, receipt_url||null, location_type||null, location_detail||null]
    );
