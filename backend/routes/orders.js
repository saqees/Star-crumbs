...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    // Notify admins via push + in-app socket + browser notification
    try {
      const userR = await pool.query('SELECT username FROM users WHERE id=$1', [req.user.id]);
      const username = userR.rows[0]?.username || 'Un usuario';
      const payload = {
        type: 'order',
      ...
      // Socket.io — real-time in-app (works in open browser tabs, no install needed)
      const io = req.app.get('io');
      if (io) io.emit('in_app_notification_admin', payload);
      // Push — for background / closed browser
