    ...
    <footer class="footer">
      <div class="container footer-grid">
        ...
        <div class="footer-actions">
          ...
          <button *ngIf="auth.isLoggedIn" class="footer-action-btn" (click)="toggleNotifSubscription()" [title]="pushService.isSubscribed()?'Desactivar notificaciones':'Activar notificaciones'">
            <i class="fas" [class.fa-bell]="!pushService.isSubscribed()" [class.fa-bell-slash]="pushService.isSubscribed()"></i>
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    effect(() => {
      ...
      if (user) {
        ...
        // Listen for real-time in-app + browser notifications via Socket.io
        setTimeout(() => {
          const sock = (this.chatService as any).socket;
          if (!sock) return;
          sock.on('in_app_notification', (data: any) => {
            this.notifService.addLocal({
              id: crypto.randomUUID(), title: data.title,
              description: data.body, is_read: false,
              created_at: new Date().toISOString(), url: data.url
            } as any);
          });
          if (user.role === 'admin') {
            sock.on('in_app_notification_admin', (data: any) => {
              this.notifService.addLocal({
                id: crypto.randomUUID(), title: data.title,
                description: data.body, is_read: false,
                created_at: new Date().toISOString(), url: data.url
  ...
  ngOnInit() {
    ...
    // Request browser notification permission after 4s (no install needed)
    setTimeout(() => {
      if (this.auth.isLoggedIn && this.notifService.canUseBrowserNotifications()
