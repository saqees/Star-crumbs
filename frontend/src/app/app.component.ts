    ...
    effect(() => {
      ...
      if (user) {
        ...
        setTimeout(() => {
          ...
          if (!sock) return;
          // Remove old listeners to avoid duplicates on re-login
          sock.off('star_crumbs_notification');
          sock.on('star_crumbs_notification', (data: any) => {
            // Add to bell counter
            this.notifService.addRealtime({
              title: data.title,
              body: data.body,
              url: data.url || '/'
            });
