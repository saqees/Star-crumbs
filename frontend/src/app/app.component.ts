    ...
    <footer class="footer">
      <div class="container footer-grid">
        ...
        <div class="footer-actions">
          <button class="footer-action-btn" (click)="shareApp()" title="Compartir">
            <i class="fas fa-share-alt"></i>
            <span>Compartir</span>
          </button>
          <button *ngIf="showInstallBanner()" class="footer-action-btn install-btn" (click)="installPWA()" title="Instalar app">
            <i class="fas fa-download"></i>
            <span>Instalar app</span>
          </button>
          <button *ngIf="auth.isLoggedIn" class="footer-action-btn notif-btn" (click)="toggleNotifSubscription()" [title]="pushService.isSubscribed()?'Desactivar notificaciones':'Activar notificaciones'">
            <i class="fas" [class.fa-bell]="!pushService.isSubscribed()" [class.fa-bell-slash]="pushService.isSubscribed()"></i>
            <span>{{pushService.isSubscribed()?'Notificaciones ✓':'Notificaciones'}}</span>
          </button>
        </div>
  ...
  installPWA() {
    ...
    this.deferredInstallPrompt.userChoice.then((choice: any) => {
      if (choice.outcome === 'accepted') {
        this.showInstallBanner.set(false);
        this.toastService.success('¡App instalada! 🎉');
      }
      this.deferredInstallPrompt = null;
    });
  }
