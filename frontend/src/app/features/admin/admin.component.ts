...
type Tab = 'products'|'categories'|'orders'|'users'|'chat'|'carousel'|'cajitas'|'why_us'|'navbar'|'footer'|'pages'|'site_reviews'|'product_reviews'|'store_theme';
  ...
  <div class="container admin-layout">
    ...
    <div class="admin-content">
      ...
      <div *ngIf="activeTab()==='cajitas'">
        <div class="ch"><h2>Cajitas / Combos</h2><button class="btn btn-primary btn-sm" (click)="openComboForm()"><i class="fas fa-plus"></i> Nueva cajita</button></div>
        <div class="combos-admin-list">
          <div *ngFor="let c of combos()" class="combo-admin-card card">
            <img [src]="c.image_url||'assets/cookie-placeholder.png'" class="combo-admin-img">
            <div class="combo-admin-info">
              <h4>{{c.name}}</h4>
              <p>{{c.description|slice:0:60}}{{(c.description?.length||0)>60?'...':''}}</p>
              <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:6px">
                <span class="vpill" style="font-size:0.7rem">×{{c.units}} unidades</span>
                <span class="vpill" style="font-size:0.7rem">$ {{c.final_price|number:'1.0-0'}}</span>
                <span *ngIf="c.discount_percent>0" class="vpill" style="font-size:0.7rem;background:#e8f5e9;color:#388e3c">-{{c.discount_percent}}%</span>
  ...
  <div *ngIf="showComboForm()" class="overlay">
    <div class="modal modal-lg">
      ...
      <div class="row2">
        ...
        <div class="form-group"><label>Unidades</label><input type="number" [(ngModel)]="comboForm.units" class="form-control" min="1" (ngModelChange)="calcComboTotal()"></div>
      </div>
      ...
      <div class="form-group">
        <label>Imagen de la cajita</label>
        <div class="fup-btn"><label class="fup-label" [class.fup-loading]="uploadingImg()"><i class="fas fa-gift"></i> {{uploadingImg()?'Subiendo...':(comboForm.image_url?'Cambiar imagen':'Subir imagen')}}<input type="file" class="fup-input" accept="image/*" (change)="onComboImageChange($event)"></label></div>
        <img *ngIf="comboForm.image_url" [src]="comboForm.image_url" style="max-width:120px;border-radius:12px;margin-top:7px">
      </div>
      <div class="form-group">
        <label>Galletas incluidas</label>
        <div *ngFor="let item of comboForm.items; let i=index" class="combo-item-row">
          <input type="text" [(ngModel)]="item.name" class="form-control" placeholder="Ej: Chocolate" style="flex:2">
          <input type="number" [(ngModel)]="item.quantity" class="form-control" placeholder="Cant." min="1" style="max-width:70px" (ngModelChange)="calcComboTotal()">
          <input type="number" [(ngModel)]="item.price" class="form-control" placeholder="Precio c/u" min="0" style="max-width:100px" (ngModelChange)="calcComboTotal()">
          <button class="ab ab-d" (click)="removeComboItem(i)"><i class="fas fa-times"></i></button>
        </div>
        <button class="btn btn-secondary btn-sm" (click)="addComboItem()"><i class="fas fa-plus"></i> Agregar galleta</button>
      </div>
      <!-- Price calculator -->
      <div class="price-calc" *ngIf="comboForm.items?.length > 0">
        <div class="row"><span>Precio base (suma galletas):</span><span>$ {{comboForm.original_price | number:'1.0-0'}}</span></div>
        <div class="row2" style="gap:14px;margin:10px 0">
          <div class="form-group" style="margin:0">
            <label style="color:rgba(255,255,255,0.8)">% Descuento</label>
            <input type="number" [(ngModel)]="comboForm.discount_percent" class="form-control" min="0" max="100" (ngModelChange)="comboForm.discount_amount=0; calcComboTotal()">
          </div>
          <div class="form-group" style="margin:0">
            <label style="color:rgba(255,255,255,0.8)">$ Descuento fijo</label>
            <input type="number" [(ngModel)]="comboForm.discount_amount" class="form-control" min="0" (ngModelChange)="comboForm.discount_percent=0; calcComboTotal()">
          </div>
        </div>
        <div class="row total-row"><span>PRECIO FINAL:</span><span>$ {{comboForm.final_price | number:'1.0-0'}}</span></div>
      </div>
      <div class="check-row mt-md">
...
export class AdminComponent implements OnInit {
  ...
  addComboItem() { this.comboForm.items=[...(this.comboForm.items||[]),{name:'',quantity:1,price:0}]; this.calcComboTotal(); }
  removeComboItem(i:number) { const items=[...this.comboForm.items]; items.splice(i,1); this.comboForm.items=items; this.calcComboTotal(); }
  calcComboTotal() {
    const base=this.comboForm.items.reduce((acc:number,i:any)=>acc+(Number(i.price)||0)*Number(i.quantity||1),0);
    this.comboForm.original_price=base;
    const disc=this.comboForm.discount_percent>0 ? base*(1-this.comboForm.discount_percent/100) : base-this.comboForm.discount_amount;
    this.comboForm.final_price=Math.max(0,Math.round(disc));
  }
  async saveCombo() {
  ...
  async onComboImageChange(event:Event) {
    const file=(event.target as HTMLInputElement).files?.[0]; if(!file) return;
    this.uploadingImg.set(true);
    const b64=await this.uploadService.fileToBase64(file);
    this.uploadService.uploadImage(b64,'star-crumbs/combos').subscribe({next:r=>{this.comboForm.image_url=r.url;this.uploadingImg.set(false);},error:()=>{this.uploadingImg.set(false);}});
  }
  getComboItems(c:any):any[] { if(!c?.items) return []; if(typeof c.items==='string'){try{return JSON.parse(c.items);}catch{return [];}} return c.items; }
