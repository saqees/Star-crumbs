  ...
  template: `
    <div class="page-enter">
      <div class="page-header" [style.background]="banner()?.bg_gradient||'linear-gradient(135deg,var(--creamy-latte),var(--almond))'">
        <div class="container">
          <h1 [style.color]="banner()?.text_color||''" [style.fontFamily]="banner()?.font?(banner()!.font+',serif'):''">{{banner()?.title||'Mis Pedidos'}} 🛍️</h1>
        </div>
      </div>
...
export class OrdersComponent implements OnInit {
  ...
  ngOnInit() { this.loadOrders('active'); this.http.get<any>(`${environment.apiUrl}/site-settings/page_banner_orders`).subscribe({next:s=>{if(s?.setting_value)this.banner.set(s.setting_value);}});
  }
