...
@Component({
    ...
    <div class="page-enter">
      ...
      <div class="container section-sm">
        ...
        <div class="profile-tabs">
          ...
          <button class="tab-pill" [class.active]="tab()==='stats'" (click)="tab.set('stats')"><i class="fas fa-chart-bar"></i> Mis Stats</button>
        </div>
        ...
        <!-- ─── STATS ─── -->
        <div *ngIf="tab()==='stats'" class="stats-tab">
          <div *ngIf="!stats()" class="stats-loading">
          ...
          <div *ngIf="stats()" class="stats-content">
            ...
            <div class="stats-cards">
              <div class="stat-card">
                ...
                <div class="stat-info">
                  <span class="stat-num">{{stats().total_units || 0}}</span>
                  <span class="stat-label">Galletas compradas</span>
                </div>
              </div>
              <div class="stat-card">
                <div class="stat-icon">📦</div>
                <div class="stat-info">
                  <span class="stat-num">{{stats().total_orders || 0}}</span>
            ...
            <!-- Favorites -->
            <div class="fav-section" *ngIf="stats().favorites?.length">
              <h4 class="fav-title">⭐ Tus galletas favoritas</h4>
              <p class="fav-sub">Las que más has pedido</p>
              <div class="fav-list">
                <div *ngFor="let p of stats().favorites; let i=index" class="fav-item">
                  <div class="fav-rank" [class]="'rank-'+i">{{i===0?'🥇':i===1?'🥈':i===2?'🥉':'#'+(i+1)}}</div>
                  <img [src]="(p.images&&p.images[0])||'assets/cookie-placeholder.png'" [alt]="p.name" class="fav-img">
                  <div class="fav-info">
                    <span class="fav-name">{{p.name}}</span>
                    <span class="fav-count">{{p.total_qty}} unidades pedidas</span>
                  </div>
                  <div class="fav-bar-wrap">
                    <div class="fav-bar" [style.width]="getFavPct(p.total_qty)+'%'"></div>
                  </div>
                </div>
              </div>
            </div>

            <div *ngIf="!stats().favorites?.length && stats().total_orders===0" class="stats-empty">
              <span>🍪</span>
              <h3>¡Aún no has pedido nada!</h3>
              <p>Haz tu primer pedido y verás tus galletas favoritas aquí.</p>
              <a routerLink="/products" class="btn btn-primary">Ver productos</a>
            </div>
          </div>
        </div>
  ...
  styles: [`
    ...
    /* Stats tab */
    .stats-tab { max-width:600px; }
    .stats-loading { padding:10px; }
    .stats-cards { display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:24px; }
    .stat-card { background:linear-gradient(135deg,var(--warm-capuchino),var(--caramel-roast)); border-radius:var(--radius-lg); padding:20px; display:flex; align-items:center; gap:14px; color:#fff; }
    .stat-icon { font-size:2.2rem; flex-shrink:0; }
    .stat-num { display:block; font-size:2rem; font-weight:700; font-family:var(--font-display); line-height:1; }
    .stat-label { font-size:.8rem; opacity:.85; }
    .fav-section { background:#fff; border-radius:var(--radius-lg); padding:22px; box-shadow:var(--shadow-sm); }
    .fav-title { color:var(--mocca-bean); margin-bottom:4px; }
    .fav-sub { font-size:.83rem; color:var(--text-light); margin-bottom:16px; }
    .fav-list { display:flex; flex-direction:column; gap:12px; }
    .fav-item { display:flex; align-items:center; gap:11px; }
    .fav-rank { font-size:1.2rem; width:32px; flex-shrink:0; text-align:center; }
    .fav-img { width:44px; height:44px; border-radius:var(--radius-md); object-fit:cover; flex-shrink:0; }
    .fav-info { flex:1; min-width:0; }
    .fav-name { display:block; font-weight:700; font-size:.88rem; color:var(--text-dark); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .fav-count { font-size:.76rem; color:var(--text-light); }
    .fav-bar-wrap { width:80px; height:6px; background:var(--almond); border-radius:3px; overflow:hidden; flex-shrink:0; }
    .fav-bar { height:100%; background:linear-gradient(90deg,var(--warm-capuchino),var(--caramel-roast)); border-radius:3px; transition:width .5s ease; }
    .stats-empty { text-align:center; padding:40px 20px; display:flex; flex-direction:column; align-items:center; gap:12px; }
    .stats-empty span { font-size:3rem; }
...
export class ProfileComponent implements OnInit {
  tab = signal<'info'|'orders'|'review'|'stats'>('info');
  ...
  ngOnInit() {
    ...
    if (u) this.form = { ...u };
    this.http.get<any>(`${environment.apiUrl}/site-settings/page_banner_profile`).subscribe({next:s=>{if(s?.setting_value)this.banner.set(s.setting_value);}});
    this.http.get<Product[]>(`${environment.apiUrl}/products`).subscribe(p => this.allProducts.set(p));
    this.http.get<any>(`${environment.apiUrl}/users/me/stats`).subscribe({next: s => this.stats.set(s), error: () => {}});
  }
  ...
  getFavPct(qty: number): number {
    const maxQty = this.stats()?.favorites?.[0]?.total_qty || 1;
