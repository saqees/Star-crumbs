    ...
    <div class="page-enter">
      <div class="page-header" [style.background]="banner()?.bg_gradient||'linear-gradient(135deg,var(--creamy-latte),var(--almond))'">
        <div class="container ph-inner">
          <div>
            <h1 [style.color]="banner()?.text_color||''" [style.fontFamily]="banner()?.font?(banner()!.font+',serif'):''">
              {{banner()?.title||'Nuestros Productos'}} <span *ngIf="!banner()?.title" class="highlight">🍪</span>
            </h1>
            <p [style.color]="banner()?.text_color||''">{{banner()?.subtitle||'Galletas artesanales hechas con amor'}}</p>
          </div>
...
export class ProductsComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get<any>(`${environment.apiUrl}/site-settings/page_banner_products`).subscribe({next:s=>{if(s?.setting_value)this.banner.set(s.setting_value);}});
    this.productService.getAll().subscribe(p => {
      // After loading, check for deep link ?open=PRODUCT_ID
      this.route.queryParams.subscribe(params => {
        if (params['open']) {
          const found = p.find((x: any) => x.id === params['open']);
          if (found) setTimeout(() => this.selectedProduct.set(found), 300);
        }
      });
      this.products.set(p);
      this.filtered.set(p);
