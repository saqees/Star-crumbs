...
export class ProductModalComponent implements OnInit, OnDestroy {
  ...
  // WhatsApp: text includes product name, qty, price AND image URL as separate link
    ...
    // Build deep-link URL to this specific product
    const baseUrl = window.location.origin;
    const productLink = `${baseUrl}/products?open=${this.product.id}`;
    ...
    // Product link (WhatsApp shows rich preview for web links)
    parts.push(``);
    parts.push(`🔗 Ver producto: ${productLink}`);
    // Image on its own line so WhatsApp previews it
    if (imgUrl) {
      parts.push(imgUrl);
    }
    parts.push(``);
