...
@Component({
  ...
  `,
  styles: [`
    .chat-overlay { position: fixed; inset: 0; z-index: 950; background: rgba(0,0,0,0.3); }
    .chat-window {
      ...
      display: flex; flex-direction: column; animation: slideUp 0.35s ease; overflow: hidden;
    }
    .chat-header {
      display: flex; align-items: center; justify-content: space-between; padding: 16px 20px;
      background: linear-gradient(135deg, var(--warm-capuchino), var(--caramel-roast)); color: #fff;
    }
    ...
    .messages-area {
      ...
      display: flex; flex-direction: column; gap: 10px; background: #f8f4f0;
    }
    .chat-welcome { text-align: center; padding: 40px 20px; display: flex; flex-direction: column; align-items: center; gap: 8px; }
    ...
    .chat-input {
      ...
      outline: none; resize: none; max-height: 80px; transition: border-color var(--transition);
    }
...
export class ChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit() {
    ...
    setInterval(() => {
      ...
      if (msgs.length > 0) {
        ...
        const alreadyExists = currentLast.some(
          (m: ChatMessage) => m.created_at === lastMsg.created_at && m.message === lastMsg.message
        );
        if (!alreadyExists) {
          this.allMessages.update((m: ChatMessage[]) => [...m, lastMsg]);
        }
      }
    }, 500);
  }
  ...
  sendMessage() {
    ...
    this.allMessages.update((m: ChatMessage[]) => [...m, msg]);
    this.messageText = '';
